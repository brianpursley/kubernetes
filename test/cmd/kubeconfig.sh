...
run_kubectl_config_set_cluster_tests() {
  ...
  kubectl config set-cluster test-cluster-1 --embed-certs --certificate-authority $ca_file
  expected=$ca_data
  actual=$(kubectl config view --raw -o jsonpath='{.clusters[?(@.name == "test-cluster-1")].cluster.certificate-authority-data}')
  if [ "$expected" != "$actual" ]; then
  ...
  kubectl config set-cluster test-cluster-2 --embed-certs --certificate-authority <(cat $ca_file)
  expected=$ca_data
...
EOF
  ...
  cert_data=$(cat $cert_file | base64 -w0)
  key_data=$(cat $key_file | base64 -w0)
